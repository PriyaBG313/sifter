...
SEC("seccomp")
int __always_inline filter_ioctl_KBASE_IOCTL_KCPU_QUEUE_CREATE(struct seccomp_data *ctx) {
    int ret = SECCOMP_RET_ALLOW;
    char dev [] = "/dev/bifrost";

    if (ctx->nr == 29 && ctx->args[1] == 0x8008802d && bpf_check_fd(dev, ctx->args[0])) {
        struct syscall_info info = {};
        info.fd = ctx->args[0];

    //arg ptr[out, kbase_ioctl_kcpu_queue_new] ptr 0xf8ffa0 8
    //arg kbase_ioctl_kcpu_queue_new kbase_ioctl_kcpu_queue_new 0xf8ffa0 8
    	struct kbase_ioctl_kcpu_queue_new v71;
    	if (bpf_probe_read_sleepable(&v71, sizeof(v71), (void *)ctx->args[2]+0) < 0)
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_KCPU_QUEUE_CREATE reject\n");
    }
    return ret;
}
