...
SEC("seccomp")
int __always_inline filter_ioctl_KBASE_IOCTL_MEM_EXEC_INIT(struct seccomp_data *ctx) {
    int ret = SECCOMP_RET_ALLOW;
    char dev [] = "/dev/bifrost";

    if (ctx->nr == 29 && ctx->args[1] == 0x40088026 && bpf_check_fd(dev, ctx->args[0])) {
        struct syscall_info info = {};
        info.fd = ctx->args[0];

    //arg ptr[in, kbase_ioctl_mem_exec_init] ptr 0xf8ffa0 8
    //arg kbase_ioctl_mem_exec_init kbase_ioctl_mem_exec_init 0xf8ffa0 8
    	struct kbase_ioctl_mem_exec_init v63;
    	if (bpf_probe_read_sleepable(&v63, sizeof(v63), (void *)ctx->args[2]+0) < 0)
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_MEM_EXEC_INIT reject\n");
    }
    return ret;
}
