...
SEC("seccomp")
int __always_inline filter_ioctl_KBASE_IOCTL_READ_USER_PAGE(struct seccomp_data *ctx) {
    int ret = SECCOMP_RET_ALLOW;
    char dev [] = "/dev/bifrost";

    if (ctx->nr == 29 && ctx->args[1] == 0xc008803c && bpf_check_fd(dev, ctx->args[0])) {
        struct syscall_info info = {};
        info.fd = ctx->args[0];

    //arg ptr[inout, kbase_ioctl_read_user_page] ptr 0xf8ffa0 8
    //arg kbase_ioctl_read_user_page kbase_ioctl_read_user_page 0xf8ffa0 8
    	struct kbase_ioctl_read_user_page v77;
    	if (bpf_probe_read_sleepable(&v77, sizeof(v77), (void *)ctx->args[2]+0) < 0)
        	return SECCOMP_RET_ERRNO | EINVAL;
    //arg const[0, user_offsets] user_offsets 0xf8ffa0 4
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_READ_USER_PAGE reject\n");
    }
    return ret;
}
