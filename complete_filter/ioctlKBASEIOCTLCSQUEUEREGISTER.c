...
SEC("seccomp")
int __always_inline filter_ioctl_KBASE_IOCTL_CS_QUEUE_REGISTER(struct seccomp_data *ctx) {
    int ret = SECCOMP_RET_ALLOW;
    char dev [] = "/dev/mali0";

    if (ctx->nr == 29 && ctx->args[1] == 0x40108024 && bpf_check_fd(dev, ctx->args[0])) {
        struct syscall_info info = {};
        info.fd = ctx->args[0];

    //arg ptr[in, base_ioctl_cs_queue_register] ptr 0xf8ffa0 8
    //arg base_ioctl_cs_queue_register base_ioctl_cs_queue_register 0xf8ffa0 24
    struct base_ioctl_cs_queue_register v69;
    if (bpf_probe_read_sleepable(&v69, sizeof(v69), (void *)ctx->args[2]+0) < 0)
        return SECCOMP_RET_ERRNO | EINVAL;
    //arg int32 int32 0xf8ffa0 4
    if (v69.buffer_size != 0x10000
        && v69.buffer_size != 0x8000) {
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_CS_QUEUE_REGISTER reject\n");
    }
    return ret;
}
