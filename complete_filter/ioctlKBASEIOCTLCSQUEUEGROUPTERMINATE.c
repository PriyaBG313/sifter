...
SEC("seccomp")
int __always_inline filter_ioctl_KBASE_IOCTL_CS_QUEUE_GROUP_TERMINATE(struct seccomp_data *ctx) {
    int ret = SECCOMP_RET_ALLOW;
    char dev [] = "/dev/bifrost";

    if (ctx->nr == 29 && ctx->args[1] == 0x4008802b && bpf_check_fd(dev, ctx->args[0])) {
        struct syscall_info info = {};
        info.fd = ctx->args[0];

    //arg ptr[in, kbase_ioctl_cs_queue_group_terminate] ptr 0xf8ffa0 8
    //arg kbase_ioctl_cs_queue_group_terminate kbase_ioctl_cs_queue_group_terminate 0xf8ffa0 8
    	struct kbase_ioctl_cs_queue_group_terminate v84;
    	if (bpf_probe_read_sleepable(&v84, sizeof(v84), (void *)ctx->args[2]+0) < 0)
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_CS_QUEUE_GROUP_TERMINATE reject\n");
    }
    return ret;
}
