...
SEC("seccomp")
int __always_inline filter_ioctl_KBASE_IOCTL_CS_EVENT_SIGNAL(struct seccomp_data *ctx) {
    int ret = SECCOMP_RET_ALLOW;
    char dev [] = "/dev/mali0";

    if (ctx->nr == 29 && ctx->args[1] == 0x802c && bpf_check_fd(dev, ctx->args[0])) {
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_CS_EVENT_SIGNAL reject\n");
    }
    return ret;
}
