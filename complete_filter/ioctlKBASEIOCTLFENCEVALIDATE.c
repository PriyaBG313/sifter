...
SEC("seccomp")
int __always_inline filter_ioctl_KBASE_IOCTL_FENCE_VALIDATE(struct seccomp_data *ctx) {
    int ret = SECCOMP_RET_ALLOW;
    char dev [] = "/dev/mali0";

    if (ctx->nr == 29 && ctx->args[1] == 0x40048019 && bpf_check_fd(dev, ctx->args[0])) {
        struct syscall_info info = {};
        info.fd = ctx->args[0];

    //arg ptr[in, kbase_ioctl_fence_validate] ptr 0xf8ffa0 8
    //arg kbase_ioctl_fence_validate kbase_ioctl_fence_validate 0xf8ffa0 4
    	struct kbase_ioctl_fence_validate v81;
    	if (bpf_probe_read_sleepable(&v81, sizeof(v81), (void *)ctx->args[2]+0) < 0)
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_FENCE_VALIDATE reject\n");
    }
    return ret;
}
