...
int __always_inline filter_ioctl_KBASE_IOCTL_CS_QUEUE_KICK(struct seccomp_data *ctx) {
    ...
    //arg ptr[in, kbase_ioctl_cs_queue_kick] ptr 0xf8ffa0 8
    //arg kbase_ioctl_cs_queue_kick kbase_ioctl_cs_queue_kick 0xf8ffa0 8
    	struct kbase_ioctl_cs_queue_kick v72;
    	if (bpf_probe_read_sleepable(&v72, sizeof(v72), (void *)ctx->args[2]+0) < 0)
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_CS_QUEUE_KICK reject\n");
    }
    return ret;
}
