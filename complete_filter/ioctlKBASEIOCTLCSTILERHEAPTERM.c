...
*/
SEC("seccomp")
int __always_inline filter_ioctl_KBASE_IOCTL_CS_TILER_HEAP_TERM(struct seccomp_data *ctx) {
    int ret = SECCOMP_RET_ALLOW;
    char dev [] = "/dev/bifrost";

    if (ctx->nr == 29 && ctx->args[1] == 0x40088031 && bpf_check_fd(dev, ctx->args[0])) {
        struct syscall_info info = {};
        info.fd = ctx->args[0];

    //arg ptr[in, kbase_ioctl_cs_tiler_heap_term] ptr 0xf8ffa0 8
    //arg kbase_ioctl_cs_tiler_heap_term kbase_ioctl_cs_tiler_heap_term 0xf8ffa0 8
    	struct kbase_ioctl_cs_tiler_heap_term v86;
    	if (bpf_probe_read_sleepable(&v86, sizeof(v86), (void *)ctx->args[2]+0) < 0)
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_CS_TILER_HEAP_TERM reject\n");
    }
    return ret;
}
