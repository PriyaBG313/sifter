...
SEC("seccomp")
int __always_inline filter_ioctl_KBASE_IOCTL_KCPU_QUEUE_ENQUEUE(struct seccomp_data *ctx) {
    int ret = SECCOMP_RET_ALLOW;
    char dev [] = "/dev/mali0";

    if (ctx->nr == 29 && ctx->args[1] == 0x4010802f && bpf_check_fd(dev, ctx->args[0])) {
        struct syscall_info info = {};
        info.fd = ctx->args[0];

    //arg ptr[in, kbase_ioctl_kcpu_queue_enqueue] ptr 0xf8ffa0 8
    //arg kbase_ioctl_kcpu_queue_enqueue kbase_ioctl_kcpu_queue_enqueue 0xf8ffa0 16
    struct kbase_ioctl_kcpu_queue_enqueue v78;
    if (bpf_probe_read_sleepable(&v78, sizeof(v78), (void *)ctx->args[2]+0) < 0)
        return SECCOMP_RET_ERRNO | EINVAL;
    //arg len len 0xf8ffa0 4
    if (v78.nr_commands != 0x1) {
    ...
    if (ret != SECCOMP_RET_ALLOW) {;
        bpf_printk("ioctl_KBASE_IOCTL_KCPU_QUEUE_ENQUEUE reject\n");
    }
    return ret;
}
